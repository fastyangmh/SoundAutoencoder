...
def _get_loss_function(project_parameters):
    if 'data_weight' in project_parameters:
        ...
        weight = None
    return nn.CrossEntropyLoss(weight=weight), nn.MSELoss()
...
class Net(LightningModule):
    ...
    def forward(self, x, y):
        x_hat, y_hat = self.backbone_model(x, y)
    ...
    def training_step(self, batch, batch_idx):
        ...
        x = torch.cat((x, y[:, None, None]), -1)
        loss = self.loss_function[0](y_hat, y)+self.loss_function[1](x_hat, x)
...
if __name__ == '__main__':
    ...
    x = torch.ones(project_parameters.batch_size, 1,
                   project_parameters.max_waveform_length)
    y = torch.randint(0, project_parameters.num_classes,
                      (project_parameters.batch_size,))
